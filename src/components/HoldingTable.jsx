...
export default function Holdingstable({ data }) {
    ...
    const handleSellOpen = (symbol, holding_id, quantity) => {
        dispatch(setSelectedSymbol(symbol));
        dispatch(handleSellModalOpen({ holding_id, quantity }));
    };
                        ...
                        {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                            ...
                            >
                                ...
                                <TableCell sx={{ display: 'flex', justifyContent: "flex-end", }}>
                                    <Button disabled={row.quantity > 0 ? false : true} variant='outlined' color="error" size="small" onClick={() => handleSellOpen(row.symbol, row._id, row.quantity)}>S</Button>
                                </TableCell>
